...
X_Tr        = 1; %Set transition location
...
% Fluid Constants:
V      =  24.78 * [0.591 0.796 1.012 1.208 1.389].^-0.5;%m/s  /0.6*0.25;% % Wind Speed - Drives Reynolds Number
mu     =  1/8 * 1.82128*10^-5;%kg/m*s  (At 17C and Sea Level - No humidity)
...
E      = 1*10^9;%Pa
v      = 0.3;
Area   = 0.000082/chord;%m^2  (Taken athickness in m by 1m width - Scaled by chord)
...
Solution    = Results;
Solution.TitlePath;
Solution.readConstants(V,rho,mu,Re,N,Cp_int,v,Area,E);
...
Nodes      = 180;   
Geom.Generate(MastType,Nodes,6412);
...
for i = 1:length(Alpha)
    ...
    while j<100
        ...
        waitbar(Progress)
        % Run CFD    
        CFD.addActionSet(0,Alpha(i),Re(i),Ma,N,0.01,X_Tr,1,200); % Run Standard Action Set in XFOIL
...
%figure(1)
hold on
Post.PlotMast(Solution,2,1,[0 0 0.7])
for i = 1:5
    Alfa = i*2-1; op_pt = find(round(Solution.Alphas(:,1))==Alfa);
    Cases = Solution.Iter(op_pt);

    Post.PlotProfile(Solution,op_pt,Cases,[1 1 1]*(1-(i*0.15)),2,4)
end
Post.hline(0);
hold off
